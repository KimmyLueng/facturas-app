"""兑换单：库存现金各币种直接互相兑换（如 USD → Bs、Bs → CNY），并生成会计凭证。

汇率严格区分委内瑞拉两种并行汇率：
  · BCV 汇率（Dólar BCV）：央行官方，会计/税务/海关法定依据 —— 默认使用。
  · 平行汇率（Dólar Paralelo）：平行市场实际成交参考 —— 仅作参考，可一键套用。
按「录入日期」取历史汇率（无历史则取当前设置兜底），都可手动修改。
...
class FxExchangePage:
    ...
    def build(self):
        ...
        # ----------------------------------------------------- 列表
        list_frame = ttk.LabelFrame(f, text="兑换单列表", padding=10)
        ...
        # ----------------------------------------------------- 表单
        form = ttk.LabelFrame(
            f, text="录入 / 编辑兑换单（库存现金各币种直接互相兑换）", padding=10)
        form.grid(row=1, column=0, sticky="ew", padx=10, pady=5)

        cur_labels = [config.currency_label(c) for c in config.PAY_CURRENCIES]
        ...
        self.vars["rate"] = tk.StringVar(value="")          # 实际成交汇率（交叉）
        self.vars["to_amount"] = tk.StringVar(value="0")
        self.vars["bcv_rate"] = tk.StringVar(value="")
        self.vars["parallel_rate"] = tk.StringVar(value="")
        ...
        ttk.Button(form, text="用 BCV", width=8,
                   command=lambda: self._apply_rate("bcv")).grid(
            row=1, column=4, sticky="w", padx=(14, 0))
        ttk.Button(form, text="用平行", width=8,
                   command=lambda: self._apply_rate("parallel")).grid(
            row=1, column=5, sticky="w", padx=8)

        ttk.Label(form, text="BCV 汇率（Dólar BCV，官方）").grid(
            row=2, column=0, sticky="w", pady=3)
        ttk.Label(form, textvariable=self.vars["bcv_rate"],
                  foreground="#1f6feb").grid(row=2, column=1, columnspan=2, sticky="w", padx=8)
        ttk.Label(form, text="平行汇率（Dólar Paralelo）").grid(
        ...
        ttk.Label(form, text="备注").grid(row=3, column=5, sticky="w", padx=(14, 0))
        ttk.Entry(form, textvariable=self.vars["notes"], width=22).grid(
            row=3, column=6, sticky="w", padx=8)
        ...
        for name in ("date", "from_currency", "to_currency"):
            self.vars[name].trace_add("write", lambda *a: self._on_date_ccy())
        self.vars["from_amount"].trace_add("write", lambda *a: self._on_amount())
        self.vars["rate"].trace_add("write", lambda *a: self._on_amount())

        self.refresh()
        self._on_date_ccy()  # 首次按今天带出 BCV/平行汇率
    ...
    def _current_cross(self, kind):
        date = self.vars["date"].get().strip()
        f = config.normalize_currency(self.vars["from_currency"].get())
    ...
    def _apply_rate(self, kind):
        val = self._current_cross(kind)
        if val:
            self.vars["rate"].set(f"{val:.4f}")
            self._recompute()

    def _recompute(self):
        """按成交汇率重算换入原币、本位币到账与汇兑损益预览。"""
        date = self.vars["date"].get().strip()
        f = config.normalize_currency(self.vars["from_currency"].get())
        t = config.normalize_currency(self.vars["to_currency"].get())
        try:
            ...
            return
        if from_amt and rate:
            new_to = from_amt * rate
            cur_to = self.vars["to_amount"].get().strip()
            if cur_to in ("", "0", "0.0", "0.00") or \
    ...
    def _on_select(self, event):
        ...
        self.vars["rate"].set(f"{float(rec.get('rate') or 0):.4f}")
    ...
    def _save(self):
        try:
            ...
            if not rec["from_amount"]:
                messagebox.showwarning("提示", "请输入兑换原币金额。", parent=self.frame)
                return
